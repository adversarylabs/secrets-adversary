...
const ruleCases = [{"key":"private-key","id":"secrets.private-key"},{"key":"aws-key","id":"secrets.aws-key"},{"key":"generic-token","id":"secrets.generic-token"}];
...
test("output ordering and protocol envelope are deterministic", async () => {
  ...
  assert.equal(envelope.result.adversary.name, "secrets");
});
